...
// Route: /api/gap-appraisal
// Feature: Appraisal
// Description: Appraisal order management (vendor ordering)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Appraisal. Appraisal order management (vendor ordering)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'appraisal', title: 'Appraisal', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'appraisal', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['appraisal']);
      return res.json({ slug: 'appraisal', items: r.rows || [] });
    }
    return res.json({ slug: 'appraisal', items: [] });
  } catch (e) {
    return res.json({ slug: 'appraisal', items: [], error: (e && e.message) || 'Server error' });
  }
});
