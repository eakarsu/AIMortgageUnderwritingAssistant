// Auto-generated feature route: borrower-engagement
// Domain: borrower engagement agent — Capability: driving proactive comms on missing docs
// Mounted under: /api/borrower-engagement
...
// POST /api/borrower-engagement/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a borrower engagement agent. Your task is driving proactive comms on missing docs. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/borrower-engagement/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/borrower-engagement/:id — fetch one
...
// DELETE /api/borrower-engagement/:id — discard a run
