// Auto-generated feature route: vision-document-intel
// Domain: mortgage doc vision agent — Capability: reading paystubs, tax returns, bank statements
// Mounted under: /api/vision-document-intel
...
// POST /api/vision-document-intel/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a mortgage doc vision agent. Your task is reading paystubs, tax returns, bank statements. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-document-intel/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-document-intel/:id — fetch one
...
// DELETE /api/vision-document-intel/:id — discard a run
