...
// Route: /api/gap-title-risk-assessment
// Feature: Title Risk Assessment
// Description: (title issue prediction)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Title Risk Assessment. (title issue prediction)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Title Risk Assessment\nDescription: (title issue prediction)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['title-risk-assessment', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'title-risk-assessment', title: 'Title Risk Assessment', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'title-risk-assessment', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['title-risk-assessment']);
      return res.json({ slug: 'title-risk-assessment', items: r.rows || [] });
    }
    return res.json({ slug: 'title-risk-assessment', items: [] });
  } catch (e) {
    return res.json({ slug: 'title-risk-assessment', items: [], error: (e && e.message) || 'Server error' });
  }
});
