// Auto-generated feature route: pipeline-bottleneck-agent
// Domain: autonomous pipeline orchestrator — Capability: auto-escalating bottlenecks
// Mounted under: /api/pipeline-bottleneck-agent
...
// POST /api/pipeline-bottleneck-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a autonomous pipeline orchestrator. Your task is auto-escalating bottlenecks. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'pipeline-bottleneck-agent',
      input: payload,
      output: parsed,
...
// GET /api/pipeline-bottleneck-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/pipeline-bottleneck-agent/:id — fetch one
...
// DELETE /api/pipeline-bottleneck-agent/:id — discard a run
