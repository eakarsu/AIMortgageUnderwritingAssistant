// Auto-generated feature route: realtime-fraud-stream
// Domain: mortgage fraud streamer — Capability: detecting fraud across application + credit + employment
// Mounted under: /api/realtime-fraud-stream
...
// POST /api/realtime-fraud-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a mortgage fraud streamer. Your task is detecting fraud across application + credit + employment. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/realtime-fraud-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/realtime-fraud-stream/:id — fetch one
...
// DELETE /api/realtime-fraud-stream/:id — discard a run
