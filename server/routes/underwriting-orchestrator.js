// Auto-generated feature route: underwriting-orchestrator
// Domain: agentic underwriting orchestrator — Capability: coordinating doc collection + conditions + risk assessment
// Mounted under: /api/underwriting-orchestrator
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Mortgage Underwriting Assistant'
      }
    };
...
// POST /api/underwriting-orchestrator/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic underwriting orchestrator. Your task is coordinating doc collection + conditions + risk assessment. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'underwriting-orchestrator',
      input: payload,
      output: parsed,
...
// GET /api/underwriting-orchestrator/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/underwriting-orchestrator/:id — fetch one
...
// DELETE /api/underwriting-orchestrator/:id — discard a run
