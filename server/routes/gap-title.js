...
// Route: /api/gap-title
// Feature: Title
// Description: Title company integration (title report automation)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Title. Title company integration (title report automation)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['title', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'title', title: 'Title', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'title', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['title']);
      return res.json({ slug: 'title', items: r.rows || [] });
    }
    return res.json({ slug: 'title', items: [] });
  } catch (e) {
    return res.json({ slug: 'title', items: [], error: (e && e.message) || 'Server error' });
  }
});
