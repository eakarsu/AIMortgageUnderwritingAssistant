...
// Route: /api/gap-pricing
// Feature: Pricing
// Description: Pricing engine (rate sheets, discount points)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Pricing. Pricing engine (rate sheets, discount points)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'pricing', title: 'Pricing', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'pricing', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['pricing']);
      return res.json({ slug: 'pricing', items: r.rows || [] });
    }
    return res.json({ slug: 'pricing', items: [] });
  } catch (e) {
    return res.json({ slug: 'pricing', items: [], error: (e && e.message) || 'Server error' });
  }
});
